//This is the controller files for the blog. 
...
//You should only create the blog 
//Blog Creation ----> 
...
try 
{
    ...
    const blog = await blogModel.blog.create({...blogInfo, ...{ "author" : authorName}, ...{"authorID" :decodedJWT.user._id }});
...
async function editBlog(req, res, next)
 {
    ...
    const update = blogModel.blog.findByIdAndUpdate(blogId, newText) 
    .then(newText => {
...
// /:id/published
   ...
   const update = await blogModel.blog.findByIdAndUpdate({_id: blogId}, newState, {new: true})
   .then(newText => {
...
async function getMyBlogs(req, res, next){
   ...
   //Find user with a particular ID and fetch all their blogs. 
     const myBlogs = await userModel.users.findOne({_id: id}).skip(skip).limit(limit);
   ...
   async function getBlogById(req, res, next) {
    ...
    const findBlog = await blogModel.blog.findOneAndUpdate({_id: id}, { $inc: { read_count: +1 } });
...
async function filterByReadCount(req, res, next){
    ...
    const limit = 20;
    const skip = (page - 1) * limit;
    const myBlogs = await blogModel.blog.find({read_count: {$gte: filterNum}}).skip(skip).limit(limit);
...
try{
    ...
    const myBlogs = await blogModel.blog.find({state: param}).skip(skip).limit(limit);
...
{
    res.json({err}, {message: "State can only be either 'published' or 'draft'"});
...
const readTime = req.query.readTime;
   try{
    ...
    const myBlogs = await blogModel.blog.find({reading_time:{$gt : readTime }}).skip(skip).limit(limit);;
...
module.exports = { createBlog, getAllBlogs, editBlog, deleteBlog, updateState, getMyBlogs, filterState, getBlogById, filterByReadCount, filterReadingTime}
