...
jest.setTimeout(20000);
  ...
  it("It should return blogs by searched author", async () => {
    const res = await request(app.app).get(`/blog/1/byauthor?author=${createdBlog.author}`)
    expect(res.statusCode).toBe(200);
  ...
  it("It Should get blogs by id from the database", async () => {
    const response = await supertest(app.app).get(`/blog/id?${createdBlog._id}`);
  ...
  it("Should get user's blogs", async () => {
    const response = await supertest(app.app)
      .get(`/user/blogs/1?secret_token=${token}`);
    expect(response.statusCode).toBe(200);
    expect(response.body.message).toStrictEqual("Fetched all blogs successfully!");
 
  })
  it("Should filter blogs by read Count ", async () => {
    const res = await request(app.app).get(`/blogs/filter/readCount?readCount=0`)
  expect(res.statusCode).toBe(200);
...
expect(res.body.message).toBe("Filter by reading time successful");
    ...
    it("Should delete a blog ", async () => {
      const res = await request(app.app).delete(`/delete/${createdBlog._id}?secret_token=${token}`)
    expect(res.statusCode).toBe(200);
  })
